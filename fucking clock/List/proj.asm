...
; 0000 0031 {
_upd_adc:
; 0000 0032 val1 = read_adc(2);
	LDI  R30,LOW(2)
	ST   -Y,R30
	RCALL _read_adc
	MOV  R4,R30
; 0000 0033 val2 = read_adc(3);
	LDI  R30,LOW(3)
	ST   -Y,R30
	RCALL _read_adc
	MOV  R5,R30
...
; 0000 0045 // Func5=In Func4=In Func3=In Func2=In Func1=In Func0=Out
; 0000 0046 // State5=T State4=T State3=T State2=T State1=T State0=1
; 0000 0047 PORTB=0x01;
	LDI  R30,LOW(1)
	OUT  0x18,R30
; 0000 0048 DDRB=0x01;
	OUT  0x17,R30
...
; 0000 004E // OC0A output: Disconnected
; 0000 004F // OC0B output: Disconnected
; 0000 0050 TCCR0A=0x00;
	LDI  R30,LOW(0)
	OUT  0x2F,R30
; 0000 0051 TCCR0B=0x00;
	OUT  0x33,R30
; 0000 0052 TCNT0=0x00;
	OUT  0x32,R30
; 0000 0053 OCR0A=0x00;
	OUT  0x36,R30
; 0000 0054 OCR0B=0x00;
	OUT  0x29,R30
; 0000 0055 
; 0000 0056 // External Interrupt(s) initialization
; 0000 0057 // INT0: Off
; 0000 0058 // Interrupt on any change on pins PCINT0-5: Off
; 0000 0059 GIMSK=0x00;
	OUT  0x3B,R30
; 0000 005A MCUCR=0x00;
	OUT  0x35,R30
; 0000 005B 
; 0000 005C // Timer/Counter 0 Interrupt(s) initialization
; 0000 005D TIMSK0=0x00;
	OUT  0x39,R30
; 0000 005E 
...
; 0000 0061 ACSR=0x80;
	LDI  R30,LOW(128)
	OUT  0x8,R30
; 0000 0062 ADCSRB=0x00;
	LDI  R30,LOW(0)
	OUT  0x3,R30
; 0000 0063 DIDR0=0x00;
	OUT  0x14,R30
; 0000 0064 
; 0000 0065 // ADC initialization
; 0000 0066 // ADC Clock frequency: 600,000 kHz
; 0000 0067 // ADC Bandgap Voltage Reference: Off
; 0000 0068 // ADC Auto Trigger Source: ADC Stopped
; 0000 0069 // Only the 8 most significant bits of
; 0000 006A // the AD conversion result are used
; 0000 006B // Digital input buffers on ADC0: On, ADC1: On, ADC2: On, ADC3: On
; 0000 006C DIDR0&=0x03;
	IN   R30,0x14
	ANDI R30,LOW(0x3)
	OUT  0x14,R30
; 0000 006D DIDR0|=0x00;
	IN   R30,0x14
	OUT  0x14,R30
; 0000 006E ADMUX=ADC_VREF_TYPE & 0xff;
	LDI  R30,LOW(32)
	OUT  0x7,R30
; 0000 006F ADCSRA=0x84;
	LDI  R30,LOW(132)
	OUT  0x6,R30
; 0000 0070 
; 0000 0071 while (1)
